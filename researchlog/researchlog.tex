...
\par \SUMMARY{A left image and right image each contain many \textit{scanlines}, each at the same vertical position. Though each image's scanline is 1-\textit{dimensional}, each point in the scanline is a square matrix of \textit{normalized} pixel values. The point \textit{i} in the left scanline is \textit{m}, }:
	}
