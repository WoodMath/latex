\chapter*{COMMONLY USED SYMBOLS AND NOTATION}

%\begin{tabular}{R{0.5cm} p{10.5cm}}
%\toprule
...
$\cong$ & \textit{Equal to to a scale factor}. Used in $\mathbf{v}\cong\mathbf{\tilde{v}}\iff\mathbf{v}=s\cdot\mathbf{\tilde{v}}$\\
$f$ & \textit{focal-length}\\
$s$ & \textit{Scalar} applied to \textit{homogeneous} vector $\mathbf{\tilde{v}}$ such that original $\mathbf{v}=s\cdot\mathbf{\tilde{v}}$ is recovered\\
