...
Each point of of interest (also referred to as a \textit{feature}) in a single image occurs in a 2-dimensional space. Unless the point is given in 3-dimensionalspace, or the point is also given in an image viewed from a different angle (commonly know as the \textit{corresponding point}), depth information is lost. The most that can be determined from the 2-dimensional information is the \textit{line of sight}, or all possible points in 3-dimensions space the point can exist while still appearing as the same point in 2-dimensional space. When viewed as   
