...
\par As previously mentioned, the point $\mathbf{x}$ in $\mathbb{R}^3$ is projected to the point $\mathbf{\tilde{y}}$ in $\mathbb{R}^{2}$ by \textit{homogenization}. This is done by setting 
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
which is interpreted as as projecting the image through the \textit{image plane} located at $z=1$. The origin at $[0,\;0,\;0]^\intercal$ is referred to as the \textit{optical center} since all light rays converge at this point. 
