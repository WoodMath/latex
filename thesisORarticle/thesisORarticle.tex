...
\par As previously mentioned, the point $\mathbf{x}$ in $\mathbb{R}^3$ is projected to the point $\mathbf{\tilde{y}}$ in $\mathbb{R}^{2}$ by \textit{homogenization}. Within this system, the \textit{origin} is referred to as the \textit{optical center} since all light rays converge at this point. The image plane containing all the projected points $\mathbf{\tilde{y}}$ is interpreted as occurring at \textit{z}-plane in which $\mathbf{x}$ iThis results implying an \textit{image-plane} at a fixed distance of 1 unit from the
